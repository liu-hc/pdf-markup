...
import { openFilePicker, saveDocument, flattenDocument, insertBlankPage, rotatePage } from '../pdf/loader';
...
const TOOL_ICONS: Record<string, string> = {
  ...
  callout: `<svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linejoin="miter" stroke-linecap="round"><rect x="7.5" y="2.5" width="8.5" height="6.5"/><line x1="7.2" y1="7.6" x2="3.2" y2="13.6"/><path d="M3.2 13.6l.5-3M3.2 13.6l3-.6" fill="none"/></svg>`,
  ...
  calibrate: `<svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><line x1="2.5" y1="13.5" x2="15.5" y2="13.5"/><line x1="2.5" y1="11.5" x2="2.5" y2="13.5"/><line x1="5.5" y1="12" x2="5.5" y2="13.5"/><line x1="8.5" y1="11" x2="8.5" y2="13.5"/><line x1="11.5" y1="12" x2="11.5" y2="13.5"/><line x1="14.5" y1="11.5" x2="14.5" y2="13.5"/><line x1="3.5" y1="5.5" x2="14.5" y2="5.5"/><polyline points="5.5,3.5 3.5,5.5 5.5,7.5"/><polyline points="12.5,3.5 14.5,5.5 12.5,7.5"/></svg>`,
  dimension: `<svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><line x1="3" y1="9" x2="15" y2="9"/><polyline points="5.5,6.5 3,9 5.5,11.5"/><polyline points="12.5,6.5 15,9 12.5,11.5"/><line x1="3" y1="5" x2="3" y2="13"/><line x1="15" y1="5" x2="15" y2="13"/></svg>`,
  measureAngle: `<svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><line x1="4" y1="14.5" x2="4" y2="3"/><line x1="4" y1="14.5" x2="15.5" y2="14.5"/><path d="M4 8.5A6 6 0 0 1 10 14.5" fill="none"/><polyline points="2.8,10 4,8.3 5.7,9.3"/><circle cx="4" cy="14.5" r="1.2" fill="currentColor" stroke="none"/></svg>`,
    ...
    <div class="main-area">
      <aside class="left-panel">
        <div class="panel-inner">
          <div class="panel-tabs"><button data-tab="bookmarks">Bookmarks</button><button data-tab="thumbnails" class="active">Thumbnail</button></div>
...
/** Width of rendered thumbnail images in CSS pixels. */
const THUMB_W = 120;
...
function renderLeftPanel(root: HTMLElement, ws: Workspace): void {
  ...
  if (doc.id !== _thumbDocId || content.querySelectorAll('.thumb-item').length !== doc.pageCount) {
    ...
    for (let i = 0; i < doc.pageCount; i++) {
      ...
      item.appendChild(document.createTextNode(`Page ${i + 1}`));
...
function renderRightPanel(root: HTMLElement): void {
  ...
  list.innerHTML = '';
  if (!doc) return;
  // Most-recent markup at the top, oldest at the bottom (markups are stored
  // in creation order, so iterate newest-first).
  ...
  for (let i = pageMarkups.length - 1; i >= 0; i--) {
    ...
    li.addEventListener('click', () => {
      import('../state/store').then(({ selectMarkups }) => selectMarkups([m.id]));
    });
    list.appendChild(li);
  }
}
...
function renderProperties(doc: ReturnType<typeof getActiveDoc>, selected: string[]): string {
  ...
  if (FILL_BEARING_TYPES.includes(m.type)) {
    ...
    fillSection = `
    <label>Infill <input type="checkbox" data-fill-enable ${fillOn ? 'checked' : ''}> <input type="color" data-prop="fillColor" value="${fillVal}" ${fillOn ? '' : 'disabled'} title="Fill color"></label>
    ${fillSwatches}`;
  }
  const weightOptions =
  ...
  return `<div class="prop-block"><strong>${m.type}</strong><p>Page ${m.pageIndex + 1}</p>
    <label>Color <input type="color" data-prop="strokeColor" value="${stroke}" title="More colors…"></label>
    ${swatches}
    ${fillSection}
    ...
    <label>Opacity <input type="range" data-prop="opacity" min="0.1" max="1" step="0.1" value="${opacity}"></label>
    ${arrowSection}
    ${textSection}
    ${measureToggle}
    ${dimSection}
